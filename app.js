...
const state={ws:null,userId:makeClientId(),userName:localStorage.getItem("0low_name")||"Diego",server:null,peers:new Map(),stream:null,inCall:false,screenTrack:null};
let toastTimer;
...
$("createBtn").addEventListener("click",()=>openModal("create"));$("joinBtn").addEventListener("click",()=>openModal("join"));$("modalClose").onclick=closeModal;$("settingsBtn").onclick=()=>settings();$("mobileGear").onclick=()=>settings();$("mobileSettings").onclick=()=>settings();
...
// Fallback diagnostics for mobile browsers.
window.addEventListener("error", function(e){
  try { toast("Erro no app: " + (e.message || "recarregue a página")); } catch(_){}
});
window.addEventListener("unhandledrejection", function(e){
  try { toast("Erro: " + (e.reason && e.reason.message ? e.reason.message : "recarregue a página")); } catch(_){}
});
